...
const handleDelete = (e) => {
  ...
  return (
    <div className="grid justify-items-start mt-16 mx-32">
      ...
      <div className="mb-16 mx-32">Signed in as @</div>
      <div className="h-48 w-2/3 border-2 border-fuchsia-400 rounded-md mx-auto text-black mb-16">
      
      <div>
      <textarea id="message" name="message" rows="4" cols="50" placeholder="Add a comment..." value={comment} onChange={(e) => setComment(e.target.value)} className="m-4 h-20 w-5/6 rounded-md pl-2 pt-2 border-2 border-gray-400 bg-gray-100"></textarea>
      </div>
          <div className="flex justify-between m-4">
            <div className="text-sm font-normal text-gray-400">{200 - comment.length} characters remaining</div>
            <div className="">
            <button className="relative inline-flex items-center justify-center p-0.5 overflow-hidden font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-purple-400 via-violet-500 to-fuchsia-500 group-hover:from-purple-400 group-hover:to-fuchsia-600 hover:text-white focus:ring-2 focus:outline-none focus:ring-purple-200 mr-4" onClick={ handleSubmit}>
      ...
      { submittedComment ? (
        <div className="">
        <h4 className="text-base font-semibold mb-2 dark:text-gray-400">Your Comment:</h4>
        <p className="text-base mb-2 dark:text-white">{submittedComment}</p>
        <div onClick={handleLike}>
        { like ? <><i className="fa-solid fa-thumbs-up text-gray-500 dark:text-white cursor-pointer hover:text-blue-500"></i> <span className="dark:text-white text-sm"> 1 Like</span>
        </> : <><i className="fa-regular fa-thumbs-up text-gray-500 dark:text-white mb-2 cursor-pointer hover:text-blue-500"></i></>}
        <span className="text-sm pl-4 cursor-pointer dark:text-white hover:text-blue-500" onClick={handleEdit}>Edit</span>
        <span className="text-sm pl-4 cursor-pointer dark:text-white hover:text-red-500" onClick={handleDelete}>Delete</span>
        </div>
        <hr />
      </div>
      ) : ( <p>
